...
1 constant nonassembler     \ 0 = use assembler for nloop and pixloop
...
: pixloop  \ assembler version. Set or clear a pixel depending on neighbour count
    ...
    height 1-  1  do     \ for each row of cells
        ...
        ['] setpix              \ [ESI+24] address of setpix
        ...
        asm{
            ...
            jne .L1             ; \ no
                ...
                jnz .L2             ; \ no
                    ...
                    call EBX            ; \ call SetPixel (via setpix)
                 ;   add ESI, 4          ; \ drop
                    jmp .L2
            .L1:
...
\ the Window procedure
 ::          \ we only need the XT for the callback, we don't need a name
    ...
    ; 20 cb: winproc              \ four parameters, give it a 20 deep stack
