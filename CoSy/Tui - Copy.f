...
: >resvar> dup R " resvar" (sym) v! ;
: resvar R " resvar" (sym) v@ ;
...
: curln ( -- a n ) | line_under_cursor 
   textwdo @ caret-y? line-text? --bc ;

: getcurln curln 2dup str addr L rplc ;
...
| \/ ` res WINDOW \/ | ===================== |
   ...
   "  res" label[  ]w
|   editbox[ ( " 320x80" ) " 330x160" size  expand  " Result " tip
|     " LUCIDA CONSOLE::10" attr: FONT 

     editbox[ | ( " 320x80" ) " 330x160"
    ...
    | { R " res.help" (sym) >resvar> v@ van rtype } key-F1-cb | Display this help
	 
	 { reswdo ins-hm } key-F11-cb
	 { reswdo ins-ymdhm } z" K_sF11" set-callback
...
| \/ ` text KEY DEFS \/ | ===================== |
     ...
     | { R " help" (sym) v@ van rtype } key-F1-cb
     { s" keyhelp" >resvar> R swap v@ >res } key-F1-cb 	| changed from ` help | 160601 | 
...
|	 { textwdo ins-ymdhm } z" K_sF11" set-callback
...
| /\ BUTTON DEFS /\ | ===================== |
  ...
  ]d CoSyDirFile van title ( " myimage"  attr: ICON ) ;
| /\ DIALOG DEF /\ | ===================== |  
