...
STACKSIZE equ 2 * 1K
TIBSIZE equ 1K
HEADERSIZE = (save_header_end - save_header)
...
slurp_exe:
	...
	mov ecx, [edi-4]	; size
	sub eax, ecx		; eax = length of  revacore portion
	lea edi, [ebx+eax]	; edi-> beginning of our area
	mov ebx, REVA
	sub ebx, [edi]
	ret
align 4 
_start:
	...
	mov esi, _s0
	call slurp_exe
; loads the compressed dictionary data and unpacks it correctly:
	...
	; EDI=> the start of the data we are interested in:
...
PROC fclass_notail
	mov byte [ssemi.do_tail], 0c3h ; ret ; 0
	jmp fclass.1
...
mylink=0
	...
	DICT '(argv)',argv,argv.dict,valclass
	DICT 'argc',argc,argc.dict,valclass
	...
	DICT 'find-dict',findinternal,findinternal.dict,dclass
...
save_header:
	; These are sanity-check values:
		dd REVA		; initial signature
	; These are data-chunks to save:
	sh_ps  dd static_start	; start of static data to persist
	sh_ssz dd static_size	; static data size to persist
	sh_ph  dd _h0		; code area
	sh_hsz dd 0		; code size (need to fill it in)
	sh_pd  dd _d0		; dictionary
	sh_dsz dd 0		; dict size (need to  fill it in)
	sh_pe  dd 0		; 'extra' data to append 
	sh_esz dd 0		; 'extra' size to append
save_header_end:

	; uncompressed size =
	;  sh_ssz + sh_hsz + sh_dsz + sh_esz
