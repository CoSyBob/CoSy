...
REVA equ 'Reva'
align 4
slurp_exe:	; loads the exe into memory, leaves [esi], eax pointing to head,
		; edi points to data after revacore, ebx is 0  if Reva signature
		; was there; returns CY if failed to load file
	upsh app_file_name
	call _ztc
	call _slurp
	jc .done
...
; loads the compressed dictionary data and unpacks it correctly:
	...
	; EAX -> size
	; ( a n )
	or ebx, ebx	; no REVA signature, this is just revacore!
	jz cold
	mov ebx, [esi]
	mov [tempX], ebx
	;add edi, 4
...
PROC _save	; ( a n -- )
; SETUP: call de-initialization routines
	call cleanup
